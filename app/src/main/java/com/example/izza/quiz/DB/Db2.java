...
import com.example.izza.quiz.Model.QuestionLeg;
import com.example.izza.quiz.Model.Rank;


public class Db2 extends SQLiteOpenHelper {

    private static String DB_NAME = "MyDB3.db";
    ...
    public Db2(Context context) {
        ...
        File file = new File(DB_PATH+"MyDB3.db");
        if(file.exists())
    ...
    //CRUD For Table
    public List<QuestionLeg> getAllQuestion() {
        List<QuestionLeg> listQuestion = new ArrayList<>();
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c;
        try {
            c = db.rawQuery("SELECT * FROM QuestionLegumes ORDER BY Random()", null);
            ...
            do {
                ...
                QuestionLeg question = new QuestionLeg(Id, Image, AnswerA, AnswerB, AnswerC, CorrectAnswer);
    ...
    public List<QuestionLeg> getQuestionMode(String mode) {
        List<QuestionLeg> listQuestion = new ArrayList<>();
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c;
        int limit = 0;
        if (mode.equals(Common.MODE.EASY.toString()))
            ...
            limit = 200;
        try {
            c = db.rawQuery(String.format("SELECT * FROM QuestionLegumes ORDER BY Random() LIMIT %d", limit), null);
            if (c == null) return null;
            c.moveToFirst();
            do {
                int Id = c.getInt(c.getColumnIndex("ID"));
