package com.example.izza.quiz.DB;
...
import com.example.izza.quiz.Common.Common;
import com.example.izza.quiz.Model.QuestionAnimal;
import com.example.izza.quiz.Model.Rank;


public class Db4 extends SQLiteOpenHelper {

    private static String DB_NAME = "MyDB4.db";
    ...
    public Db4(Context context) {
        ...
        File file = new File(DB_PATH+"MyDB4.db");
        if(file.exists())
    ...
    //CRUD For Table
    public List<QuestionAnimal> getAllQuestion() {
        List<QuestionAnimal> listQuestion = new ArrayList<>();
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c;
        try {
            c = db.rawQuery("SELECT * FROM QuestionAnimal ORDER BY Random()", null);
            ...
            do {
                ...
                QuestionAnimal question = new QuestionAnimal(Id, Image, AnswerA, AnswerB, AnswerC, CorrectAnswer);
    ...
    public List<QuestionAnimal> getQuestionMode(String mode) {
        List<QuestionAnimal> listQuestion = new ArrayList<>();
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c;
        int limit = 0;
        if (mode.equals(Common.MODE.EASY.toString()))
            ...
            limit = 200;
        try {
            c = db.rawQuery(String.format("SELECT * FROM QuestionAnimal ORDER BY Random() LIMIT %d", limit), null);
            if (c == null) return null;
            c.moveToFirst();
            do {
                int Id = c.getInt(c.getColumnIndex("ID"));
